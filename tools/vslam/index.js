...
function record_waypoints_handler(tmp_img){
	...
	m_client.publish('pserver-vslam-info', JSON.stringify({
		"mode" : "RECORD",
		"state" : "RECORDING",
...
function update_auto_drive_cur(cur) {
	...
	m_client.set('pserver-vslam-cur', m_auto_drive_cur).then((data) => {
		console.log('set auto drive cur ' + cur, data);
	});
}
function update_auto_drive_waypoints(waypoints) {
	m_auto_drive_waypoints = waypoints;
	m_client.set('pserver-vslam-waypoints', JSON.stringify(waypoints)).then((data) => {
...
function main() {
	...
	subscriber.connect().then(() => {
		...
		subscriber.subscribe('pserver-vslam', (data, key) => {
