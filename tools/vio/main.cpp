 ...
 */
 ...
 public:
   ...
   bool pop(T& out) {
     ...
     cv.wait(lk, [&]{ return !q.empty() || terminate; });
 ...
 void imu_thread() {
   ...
   while (redisGetReply(c, (void**)&r) == REDIS_OK) {
     ...
     ImuMsg m;
     m.t_ns = j["t_ns"];
     m.acc  = Eigen::Vector3d(j["acc"][0],  j["acc"][1],  j["acc"][2]);
     m.gyro = Eigen::Vector3d(j["gyro"][0], j["gyro"][1], j["gyro"][2]);
...
static cv::Mat decode_image_from_base64(const std::string& b64)
{
 ...
 int main(int argc, char** argv) {
   ...
   vio_config.load("vio_config.yaml");
