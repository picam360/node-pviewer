...
// This implementation assumes it publishes cumulative counts for left/right wheels.
 ...
 // ============================================================
// EKF state:
//   [px, py, pz, yaw, pitch, roll, encoder_vio_timeoffset_ns]
 // ============================================================
 struct EkfState
 {
     ...
     EkfState()
     {
         ...
         P.setIdentity();
         P *= 1e-2;
         P(6,6) = 1e16; // timeoffset variance
     }
 };
 ...
 public:
     ...
     void predict_from_encoder(int64_t enc_t_ns, double v_mps, double yawrate_rps)
     {
        ...
        // State indices
        const int PX=0, PY=1, PZ=2, YAW=3, PITCH=4, ROLL=5, TOFF=6;
        ...
        // (Optional) timeoffset are random walk -> keep identity.
    ...
    // Uses VIO position + yaw/pitch/roll from quaternion.
                          ...
                          const Eigen::Quaterniond& q_w_i)
     {
        ...
        // EKF update
         ...
         Eigen::Matrix<double,7,6> K = st_.P * H.transpose() * S.inverse();
                               ...
                               const Eigen::Quaterniond& q_w_i)
     {
        ...
        // shrink covariance after init
         ...
         st_.P.diagonal() <<
             0.1, 0.1, 0.2,
             0.1, 0.2, 0.2,
             1e16; // keep timeoffset uncertain initially
     }
