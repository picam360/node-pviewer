 ...
 */
 ...
 public:
     int64_t convert(uint64_t ts_us)
     {
         ...
         if (!initialized_)
         {
             ...
             base_epoch_ns_ = (int64_t)now.tv_sec * 1000000000LL + (int64_t)now.tv_usec * 1000LL;
             base_ts_us_ = ts_us;
             initialized_ = true;
 ...
 // ============================================================
 static TsUsToEpoch g_tsconv_imu;
                                            ...
                                            std::vector<ImuSample> &out_list)
 {
     ...
     for (const auto &item : j)
     {
         ...
         ImuSample out;
         out.t_ns = g_tsconv_imu.convert(ts_us);
 ...
 static TsUsToEpoch g_tsconv_enc;
                                            ...
                                            std::vector<EncOdom> &out_list)
 {
     ...
     for (const auto &item : j)
     {
         ...
         // convert to ns
         out.t_ns = g_tsconv_enc.convert(ts_us);
